	...
	namespace details
	{
		...
		struct Operator
		{
			...
			static Ret write( char const * const p_operator, T const & lhs, U const & rhs )
			{
				...
				result.m_value << std::string( lhs ) << ' ' << p_operator << ' ' << toString( rhs );
				return result;
			}
		};
		...
		struct Operator< Optional< TypeR >, Optional< TypeA >, TypeB >
		{
			...
			static Ret write( char const * const p_operator, T const & lhs, U const & rhs )
			{
				Ret result( findWriter( lhs, rhs ), std::string(), lhs.isEnabled() );
				result.m_value << std::string( lhs ) << ' ' << p_operator << ' ' << toString( rhs );
				return result;
			}
		};
		...
		struct Operator< Optional< TypeR >, TypeA, Optional< TypeB > >
		{
			...
			static Ret write( char const * const p_operator, T const & lhs, U const & rhs )
			{
				Ret result( findWriter( lhs, rhs ), std::string(), rhs.isEnabled() );
				result.m_value << std::string( lhs ) << ' ' << p_operator << ' ' << toString( rhs );
				return result;
			}
		};
...
#	define GLSL_IMPLEMENT_BOOLEAN_OPERATOR( LhsType, RhsType, Operator )\
	...
	{
		return writeFunctionCall< Value >( x.m_container, cuT( "pow" ), x, y );
	}
	...
	{
		return writeFunctionCall< Value >( p.m_container, cuT( "pow2" ), p );
	}
	...
	{
		return writeFunctionCall< Value >( x.m_container, cuT( "mod" ), x, y );
	}
