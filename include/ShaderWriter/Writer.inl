namespace sdw
{
	...
	void GlslWriter::writeAssign( Type const & lhs, Optional< T > const & rhs )
	{
		if ( rhs.isEnabled() )
		{
			m_stream << std::string( lhs ) << cuT( " = " ) << std::string( rhs ) << cuT( ";" ) << std::endl;
		}
	}

	template< typename RetType, typename ... Params >
	inline Function< RetType, Params... > GlslWriter::implementFunction( std::string const & name
	...
	{
		m_stream << cuT( "return " ) << std::string( result ) << cuT( ";" ) << std::endl;
	}
	...
	ExprType GlslWriter::paren( ExprType const expr )
	{
		...
		result.m_value << cuT( "( " ) << std::string( expr ) << cuT( " )" );
		return result;
	}
	...
	{
		T result;
		result.m_value << castor::string::trim( result.m_type ) << cuT( "( " ) << std::string( from ) << cuT( " )" );
		return result;
	}
#pragma region Constant declaration
	...
	*/
	/**@{*/
	template< typename T >
	inline T GlslWriter::declConstant( std::string const & name
		, T const & rhs )
	{
		using Type = typename TypeOf< T >::Type;
		registerConstant( name, TypeTraits< Type >::TypeEnum );
		*this << "#define " << name << " ";
		m_stream << std::string( rhs ) << std::endl;
		return T( this, name );
	}

	template< typename T >
	inline Optional< T > GlslWriter::declConstant( std::string const & name
		, T const & rhs
		, bool enabled )
	{
		using Type = typename TypeOf< T >::Type;
...
#pragma region Specialisation constant declaration
	...
	*/
	/**@{*/
	template< typename T >
	inline T GlslWriter::declSpecConstant( std::string const & name
		, uint32_t location
		, T const & rhs )
	{
		using Type = typename TypeOf< T >::Type;
	...
	template< typename T >
	inline Optional< T > GlslWriter::declSpecConstant( std::string const & name
		, uint32_t location
		, T const & rhs
		, bool enabled )
	{
...
#pragma region Sampler declaration
	...
	*/
	/**@{*/
	template< typename T >
	inline T GlslWriter::declSampler( std::string const & name
		, uint32_t binding )
	{
		using Type = typename TypeOf< T >::Type;
	...
	template< typename T >
	inline Optional< T > GlslWriter::declSampler( std::string const & name
		, uint32_t binding
		, bool enabled )
	{
		using Type = typename TypeOf< T >::Type;
	...
	template< typename T >
	inline Array< T > GlslWriter::declSamplerArray( std::string const & name
		, uint32_t binding
		, uint32_t dimension )
	{
		using Type = typename TypeOf< T >::Type;
	...
	template< typename T >
	inline Optional< Array< T > > GlslWriter::declSamplerArray( std::string const & name
		, uint32_t binding
		, uint32_t dimension
		, bool enabled )
	{
...
#pragma region Uniform Declaration
	...
	*/
	/**@{*/
	template< typename T >
	inline T GlslWriter::declUniform( std::string const & name
		, uint32_t location )
	{
		using Type = typename TypeOf< T >::Type;
		...
		, T const & rhs )
	{
		...
		*this << m_keywords->getUniformLayout( location ) << Uniform{} << TypeTraits< Type >::Name << " " << name << cuT( " = " );
		m_stream << std::string( rhs ) << cuT( ";" ) << std::endl;
		return T( this, name );
	}

	template< typename T >
	inline Optional< T > GlslWriter::declUniform( std::string const & name
		, uint32_t location
		, bool enabled )
	{
		using Type = typename TypeOf< T >::Type;
	...
	template< typename T >
	inline Array< T > GlslWriter::declUniformArray( std::string const & name
		, uint32_t location
		, uint32_t dimension )
	{
		using Type = typename TypeOf< T >::Type;
		...
		, std::vector< T > const & rhs )
	{
		...
		*this << m_keywords->getUniformLayout( location ) << Uniform{} << TypeTraits< Type >::Name << " " << name << cuT( "[" ) << dimension << cuT( "] = " ) << TypeTraits< Type >::Name << " " << cuT( "[]( " );
		...
		{
			*this << sep << std::string( value ) << endl;
			sep = cuT( "\t, " );
		}
	...
	template< typename T >
	inline Optional< Array< T > > GlslWriter::declUniformArray( std::string const & name
		, uint32_t location
		, uint32_t dimension
		, bool enabled )
	{
...
#pragma region Input declaration
	...
	*/
	/**@{*/
	template< typename T >
	inline T GlslWriter::declAttribute( std::string const & name
		, uint32_t location )
	{
		using Type = typename TypeOf< T >::Type;
	...
	template< typename T >
	inline Optional< T > GlslWriter::declAttribute( std::string const & name
		, uint32_t location
		, bool enabled )
	{
		using Type = typename TypeOf< T >::Type;
	...
	template< typename T >
	inline Array< T > GlslWriter::declAttributeArray( std::string const & name
		, uint32_t location
		, uint32_t dimension )
	{
		using Type = typename TypeOf< T >::Type;
	...
	template< typename T >
	inline Optional< Array< T > > GlslWriter::declAttributeArray( std::string const & name
		, uint32_t location
		, uint32_t dimension
		, bool enabled )
	{
	...
	template< typename T >
	inline T GlslWriter::declInput( std::string const & name
		, uint32_t location )
	{
		using Type = typename TypeOf< T >::Type;
		registerInput( name, location, TypeTraits< Type >::TypeEnum );
	...
	template< typename T >
	inline Optional< T > GlslWriter::declInput( std::string const & name
		, uint32_t location
		, bool enabled )
	{
		using Type = typename TypeOf< T >::Type;
	...
	template< typename T >
	inline Array< T > GlslWriter::declInputArray( std::string const & name
		, uint32_t location
		, uint32_t dimension )
	{
		using Type = typename TypeOf< T >::Type;
	...
	template< typename T >
	inline Optional< Array< T > > GlslWriter::declInputArray( std::string const & name
		, uint32_t location
		, uint32_t dimension
		, bool enabled )
	{
...
#pragma region Output declaration
	...
	*/
	/**@{*/
	template< typename T >
	inline T GlslWriter::declFragData( std::string const & name
		, uint32_t location )
	{
		using Type = typename TypeOf< T >::Type;
	...
	template< typename T >
	inline T GlslWriter::declOutput( std::string const & name
		, uint32_t location )
	{
		using Type = typename TypeOf< T >::Type;
		registerOutput( name, location, TypeTraits< Type >::TypeEnum );
	...
	template< typename T >
	inline Optional< T > GlslWriter::declOutput( std::string const & name
		, uint32_t location
		, bool enabled )
	{
		using Type = typename TypeOf< T >::Type;
	...
	template< typename T >
	inline Array< T > GlslWriter::declOutputArray( std::string const & name
		, uint32_t location
		, uint32_t dimension )
	{
		using Type = typename TypeOf< T >::Type;
	...
	template< typename T >
	inline Optional< Array< T > > GlslWriter::declOutputArray( std::string const & name
		, uint32_t location
		, uint32_t dimension
		, bool enabled )
	{
...
#pragma region Locale declaration
	...
	*/
	/**@{*/
	template< typename T >
	inline T GlslWriter::declLocale( std::string const & name )
	{
		using Type = typename TypeOf< T >::Type;
		...
		, T const & rhs )
	{
		using Type = typename TypeOf< T >::Type;
		registerName( name, TypeTraits< Type >::TypeEnum );
		m_stream << TypeTraits< Type >::Name << " " << name << cuT( " = " ) << std::string( rhs ) << cuT( ";" ) << std::endl;
		T result( this, name );
		return result;
	}

	template< typename T >
	inline Optional< T > GlslWriter::declLocale( std::string const & name
		, bool enabled )
	{
		using Type = typename TypeOf< T >::Type;
		registerName( name, TypeTraits< Type >::TypeEnum );
	...
	template< typename T >
	inline Array< T > GlslWriter::declLocaleArray( std::string const & name
		, uint32_t dimension )
	{
		using Type = typename TypeOf< T >::Type;
		registerName( name, TypeTraits< Type >::TypeEnum );
	...
	template< typename T >
	inline Optional< Array< T > > GlslWriter::declLocaleArray( std::string const & name
		, uint32_t dimension
		, bool enabled )
	{
		using Type = typename TypeOf< T >::Type;
...
#pragma region Built-in declaration
	...
	*/
	/**@{*/
	template< typename T >
	inline T GlslWriter::declBuiltin( std::string const & name )
	{
	...
	template< typename T >
	inline Optional< T > GlslWriter::declBuiltin( std::string const & name
		, bool enabled )
	{
		registerName( name, TypeTraits< T >::TypeEnum );
		return Optional< T >( this, name, enabled );
	...
	template< typename T >
	inline Array< T > GlslWriter::declBuiltinArray( std::string const & name
		, uint32_t dimension )
	{
		registerName( name, TypeTraits< T >::TypeEnum );
		return Array< T >( this, name, dimension );
	...
	template< typename T >
	inline Optional< Array< T > > GlslWriter::declBuiltinArray( std::string const & name
		, uint32_t dimension
		, bool enabled )
	{
		registerName( name, TypeTraits< T >::TypeEnum );
	...
	template< typename T >
	inline T GlslWriter::getBuiltin( std::string const & name )
	{
	...
	template< typename T >
	inline Optional< T > GlslWriter::getBuiltin( std::string const & name
		, bool enabled )
	{
		checkNameExists( name, TypeTraits< T >::TypeEnum );
		return Optional< T >( this, name, enabled );
	...
	template< typename T >
	inline Array< T > GlslWriter::getBuiltinArray( std::string const & name )
	{
		checkNameExists( name, TypeTraits< T >::TypeEnum );
		return Array< T >( this, name, 0xFFFFFFFF );
	}
	...
	template< typename T >
	inline Optional< Array< T > > GlslWriter::getBuiltinArray( std::string const & name
		, bool enabled )
	{
		checkNameExists( name, TypeTraits< T >::TypeEnum );
		return Optional< Array< T > >( this, name, 0xFFFFFFFF, enabled );
