...
#define DummyMain writer.implementMain( [&]( sdw::FragmentIn in, sdw::FragmentOut out ){} )
	...
	void testStructuredSsbo( test::sdw_test::TestCounts & testCounts )
	{
		astTestBegin( "testStructuredSsbo" + ast::debug::getTypeName( sdw::typeEnumV< T > ) );
		{
			...
			astCheck( static_cast< sdw::stmt::ShaderStructBufferDecl const & >( stmt ).getBindingPoint() == 1u );
			astCheck( static_cast< sdw::stmt::ShaderStructBufferDecl const & >( stmt ).getDescriptorSet() == 1u );
			DummyMain;
	...
	void testArraySsbo( test::sdw_test::TestCounts & testCounts )
	{
		astTestBegin( "testArraySsbo" + ast::debug::getTypeName( sdw::typeEnumV< T > ) );
		{
			...
			astCheck( getNonArrayKind( value.getType() ) == sdw::typeEnumV< T > );
			astCheck( getArraySize( value.getType() ) == sdw::type::NotArray );
			astRequire( value.getExpr()->getKind() == sdw::expr::Kind::eArrayAccess );
			auto & stmt = *shader.getStatements()->back();
			astRequire( stmt.getKind() == sdw::stmt::Kind::eShaderStructBufferDecl );
			astCheck( static_cast< sdw::stmt::ShaderStructBufferDecl const & >( stmt ).getBindingPoint() == 1u );
			astCheck( static_cast< sdw::stmt::ShaderStructBufferDecl const & >( stmt ).getDescriptorSet() == 1u );
