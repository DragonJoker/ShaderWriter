...
namespace sdw
{
	...
	void GlslWriter::registerName( std::string const & name, type::Kind type )
	{
	...
	void GlslWriter::checkNameExists( std::string const & name, type::Kind type )
	{
		auto it = m_registered.find( name );
	...
	{
		m_container.setSource( m_stream.str() );
		return m_container;
	}
	...
	{
		m_stream << std::string( lhs ) << cuT( " = " ) << castor::string::toString( rhs, 10, Expr::getLocale() ) << cuT( ";" ) << std::endl;
	}

	void GlslWriter::writeAssign( Type const & lhs, unsigned int const & rhs )
	{
		m_stream << std::string( lhs ) << cuT( " = " ) << castor::string::toString( rhs, 10, Expr::getLocale() ) << cuT( "u;" ) << std::endl;
	}

	void GlslWriter::writeAssign( Type const & lhs, float const & rhs )
	{
		m_stream << std::string( lhs ) << cuT( " = " ) << castor::string::toString( rhs, Expr::getLocale() ) << cuT( ";" ) << std::endl;
	}
	...
	Struct GlslWriter::getStruct( std::string const & name )
	{
	...
	GlslWriter & GlslWriter::operator<<( std::string const & rhs )
	{
		m_stream << rhs;
		return *this;
	}
