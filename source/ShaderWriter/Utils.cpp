	...
	void Utils::declareGetMapNormal()
	{
		m_getMapNormal = m_writer.implementFunction< Vec3 >( "getMapNormal"
			, [&]( Vec2 const & uv
				...
				, Vec3 const & position )
			{
				auto c3d_mapNormal( m_writer.getVariable< SampledImage2D >( "c3d_mapNormal" ) );
	...
	void Utils::declareComputeIBL()
	{
		m_computeIBL = m_writer.implementFunction< Vec3 >( "computeIBL"
			, [&]( Vec3 const & normal
				...
				, Vec3 const & worldEye
				, SampledImageCube const & irradianceMap
				, SampledImageCube const & prefilteredEnvMap
				, SampledImage2D const & brdfMap )
			{
			...
			, InSampledImageCube{ m_writer, "irradianceMap" }
			, InSampledImageCube{ m_writer, "prefilteredEnvMap" }
			, InSampledImage2D{ m_writer, "brdfMap" } );
	}
