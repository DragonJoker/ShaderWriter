...
namespace spirv
{
	...
	namespace debug
	{
			...
			, ast::stmt::InOutVariableDecl const & stmt )
		{
			...
			result += getLocationName( *stmt.getVariable() ) + "=" + debug::writeValue( stmt.getLocation() );
			sep = ", ";

			if ( shaderStage == ast::ShaderStage::eGeometry
				&& stmt.getVariable()->isGeometryStream() )
			{
				result += sep + "stream=" + debug::writeValue( stmt.getStreamIndex() );
				sep = ", ";
			}
			...
			{
				result += sep + "index=" + debug::writeValue( stmt.getBlendIndex() );
			}
		...
		void incLine( DebugStatement & dbgStmt )
		{
			dbgStmt.text += "\n";
			++dbgStmt.lineEnd;
		}

		static void writeBlock( std::string indent
			, ast::type::Struct const & structType
			, DebugStatement & dbgStmt )
		{
			dbgStmt.text += structType.getName();
			...
			if ( !structType.empty() )
			{
				...
				for ( auto & mbr : structType )
				{
					...
					{
						dbgStmt.text += "layout( location=" + debug::writeValue( mbr.location ) + " ) ";
						dbgStmt.text += ( structType.isShaderInput()
							? std::string{ "in" }
							: std::string{ "out" } ) + " ";
					}

					dbgStmt.text += getTypeName( mbr.type ) + " " + mbr.name;
					dbgStmt.text += getTypeArraySize( mbr.type );
					dbgStmt.text += ";";
					incLine( dbgStmt );
				}
		...
		static void writeStruct( std::string const & indent
			, ast::type::Struct const & structType
			, std::string const & instanceName
			, DebugStatement & dbgStmt )
		{
			dbgStmt.text += indent + "struct ";
			writeBlock( indent, structType, dbgStmt );

			if ( !instanceName.empty() )
			{
				dbgStmt.text += " " + instanceName;
			}
		...
		static std::string getGlslName( ast::expr::CombinedImageAccess value )
		{
			std::string result;
		...
		static std::string getGlslName( ast::expr::Intrinsic value )
		{
			std::string result;
		...
		inline std::string getGlslName( ast::expr::StorageImageAccess value )
		{
			std::string result;
		...
		private:
			ExprVisitor( DebugStatement & result )
				: m_result{ result }
			{
			}
			...
			{
				wrap( expr->getLHS() );
				m_result.text += " " + getOperatorName( expr->getKind() ) + " ";
			...
			void visitAggrInitExpr( ast::expr::AggrInit * expr )override
			{
				if ( expr->getIdentifier() )
				{
					...
					m_result.text += getTypeName( expr->getType() ) + " ";
					doSubmit( expr->getIdentifier() );
					m_result.text += getTypeArraySize( expr->getType() );
					m_result.text += " = ";
				}
			...
			void visitCompositeConstructExpr( ast::expr::CompositeConstruct * expr )override
			{
				...
				{
					m_result.text += getCtorName( *expr->getArgList()[0]
						, *expr->getArgList()[1] );
				}
				else
				{
					m_result.text += getCtorName( expr->getComposite(), getScalarType( expr->getComponent() ) );
				}
			...
			void visitMbrSelectExpr( ast::expr::MbrSelect * expr )override
			{
				...
				{
					m_result.text += "gl_" + getName( mbr.builtin );
				}
				else
				{
					m_result.text += mbr.name;
				}
			}
			...
			{
				m_result.text += visitVariable( expr->getVariable() );
			}

			void visitImageAccessCallExpr( ast::expr::StorageImageAccessCall * expr )override
			{
				m_result.text += getGlslName( expr->getImageAccess() ) + "(";
				std::string sep;
			...
			void visitInitExpr( ast::expr::Init * expr )override
			{
				...
				m_result.text += getTypeName( expr->getType() ) + " ";
				doSubmit( expr->getIdentifier() );
				m_result.text += getTypeArraySize( expr->getIdentifier()->getType() );
				m_result.text += " = ";
			...
			void visitIntrinsicCallExpr( ast::expr::IntrinsicCall * expr )override
			{
					...
					|| expr->getIntrinsic() == ast::expr::Intrinsic::eMemoryBarrier )
				{
					...
					if ( memory == ast::type::Scope::eWorkgroup )
					{
						...
						{
							m_result.text = "groupMemoryBarrier()";
						}
						else
						{
							m_result.text = "barrier()";
						}
					}
					else if ( memory == ast::type::Scope::eSubgroup )
					{
						if ( checkAllMemoryBarrier( semantics ) )
						{
							if ( isControlBarrier )
							{
								m_result.text = "subgroupBarrier()";
							}
							else
							{
								m_result.text = "subgroupMemoryBarrier()";
							}
						}
						else if ( checkBufferMemoryBarrier( semantics ) )
						{
							m_result.text = "subgroupMemoryBarrierBuffer()";
						}
						else if ( checkSharedMemoryBarrier( semantics ) )
						{
							m_result.text = "subgroupMemoryBarrierShared()";
						}
						else if ( checkImageMemoryBarrier( semantics ) )
						{
							m_result.text = "subgroupMemoryBarrierImage()";
						}
						else
						{
							m_result.text = "subgroupBarrier()";
					...
					else
					{
						if ( checkAllMemoryBarrier( semantics ) )
						{
							m_result.text = "memoryBarrier()";
						}
						else if ( checkBufferMemoryBarrier( semantics ) )
						{
							m_result.text = "memoryBarrierBuffer()";
						}
						else if ( checkSharedMemoryBarrier( semantics ) )
						{
							m_result.text = "memoryBarrierShared()";
						}
						else if ( checkImageMemoryBarrier( semantics ) )
						{
							m_result.text = "memoryBarrierImage()";
						}
						else
						{
							m_result.text = "barrier()";
				...
				{
					m_result.text += getGlslName( expr->getIntrinsic() ) + "(";
					std::string sep;
			...
			{
				wrap( expr->getOuterExpr() );
				m_result.text += "." + getName( expr->getSwizzle() );
			}

			void visitCombinedImageAccessCallExpr( ast::expr::CombinedImageAccessCall * expr )override
			{
				m_result.text += getGlslName( expr->getCombinedImageAccess() ) + "(";
				std::string sep;
		...
		public:
			static DebugStatements submit( ast::ShaderStage shaderStage
				, ast::stmt::Stmt * stmt )
			{
				DebugStatements result{ &stmt->getStmtCache().getAllocator() };
				std::string indent;
				submit( shaderStage, stmt, indent, result );
				return result;
			}
		...
		private:
			...
			{
				StmtDebugVisitor vis{ shaderStage, std::move( indent ), result };
				stmt->accept( &vis );
			}
			...
			void doSubmit( ast::expr::Expr * expr )
			{
				debug::ExprVisitor::submit( expr, m_current );
			}

			void doAppendLineEnd()
			{
				if ( m_appendLineEnd && m_current.lineBegin > 0u )
				{
			...
			void doNextStatement( ast::stmt::Stmt * stmt )
			{
				++m_current.lineEnd;
				auto line = m_current.lineEnd;
				m_current.stmt = stmt;
				m_result.push_back( std::move( m_current ) );
				m_current.lineBegin = line;
				m_current.lineEnd = line;
			}

			void visitContainerStmt( ast::stmt::Container * stmt )
			{
				for ( auto & curStmt : *stmt )
				{
					curStmt->accept( this );
					doNextStatement( curStmt.get() );
				}
			}

			void visitBreakStmt( ast::stmt::Break * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent + "break;";
			}

			void visitContinueStmt( ast::stmt::Continue * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent + "continue;";
			}

			void visitConstantBufferDeclStmt( ast::stmt::ConstantBufferDecl * stmt )
			{
				if ( !stmt->empty() )
				{
					m_appendLineEnd = true;
					doAppendLineEnd();
					m_current.text += m_indent;
					m_current.text += "layout(";
					m_current.text += debug::getName( stmt->getMemoryLayout() );
					doWriteBinding( stmt->getBindingPoint()
						, stmt->getDescriptorSet()
						, ", " );

					m_current.text += ") uniform " + stmt->getName();
			...
			void visitDemoteStmt( ast::stmt::Demote * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent + "demote;";
			}

			void visitDispatchMeshStmt( ast::stmt::DispatchMesh * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent + "EmitMeshTasksEXT";
				m_current.text += "(";
			...
			void visitTerminateInvocationStmt( ast::stmt::TerminateInvocation * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent + "terminate;";
			}

			void visitPushConstantsBufferDeclStmt( ast::stmt::PushConstantsBufferDecl * stmt )
			{
				if ( !stmt->empty() )
				{
					m_appendLineEnd = true;
					doAppendLineEnd();
					m_current.text += m_indent;
					m_current.text += "layout(push_constant) ";
					m_current.text += "uniform " + stmt->getName();
					m_appendSemiColon = true;
					visitCompoundStmt( stmt );
					m_appendLineEnd = true;
			...
			void visitCommentStmt( ast::stmt::Comment * stmt )
			{
			}

			void visitCompoundStmt( ast::stmt::Compound * stmt )
			{
				doAppendLineEnd();
				incLine( m_current );
				m_current.text += m_indent + "{";
				doNextStatement( stmt );
				auto save = m_indent;
				m_indent += "\t";
			...
			void visitDoWhileStmt( ast::stmt::DoWhile * stmt )
			{
				m_appendLineEnd = true;
				doAppendLineEnd();
				m_current.text += m_indent + "do";
				...
				{
					m_current.text += m_indent + "while (bool(";
					doSubmit( stmt->getCtrlExpr() );
					m_current.text += "));";
				}
				else
				{
					m_current.text += m_indent + "while (";
					doSubmit( stmt->getCtrlExpr() );
					m_current.text += ");";
				}
			...
			void visitElseIfStmt( ast::stmt::ElseIf * stmt )
			{
				if ( stmt->getCtrlExpr()->getType()->getKind() != ast::type::Kind::eBoolean )
				{
					m_current.text += m_indent + "else if (bool(";
					doSubmit( stmt->getCtrlExpr() );
					m_current.text += "))";
				}
				else
				{
					m_current.text += m_indent + "else if (";
					doSubmit( stmt->getCtrlExpr() );
					m_current.text += ")";
				}
			...
			void visitForStmt( ast::stmt::For * stmt )
			{
				m_appendLineEnd = true;
				doAppendLineEnd();
				m_current.text += m_indent + "for (";
			...
			void visitFragmentLayoutStmt( ast::stmt::FragmentLayout * stmt )
			{
				std::string origin = debug::getName( stmt->getFragmentOrigin() );
				std::string center = debug::getName( stmt->getFragmentCenter() );

				if ( !origin.empty() || !center.empty() )
				{
					m_current.text += "layout(";
					m_current.text += origin;
					m_current.text += ( ( origin.empty() || center.empty() )
						? std::string{}
					: std::string{ ", " } );
					m_current.text += center;
					m_current.text += ") in vec4 gl_FragCoord;";
				}
			}

			void visitFunctionDeclStmt( ast::stmt::FunctionDecl * stmt )
			{
				m_appendLineEnd = true;
				doAppendLineEnd();
				auto type = stmt->getType();
				m_current.text += m_indent + debug::getTypeName( type->getReturnType() );
				m_current.text += " " + stmt->getName() + "(";
				std::string sep;

				for ( auto & param : *type )
				{
					m_current.text += sep + debug::getDirectionName( *param )
						+ " " + debug::getTypeName( param->getType() )
						+ " " + param->getName() + debug::getTypeArraySize( param->getType() );
					sep = ", ";
				}
			...
			void visitAccelerationStructureDeclStmt( ast::stmt::AccelerationStructureDecl * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent;
				m_current.text += "layout(";
				doWriteBinding( stmt->getBindingPoint()
					, stmt->getDescriptorSet()
					, "" );
				m_current.text += ") uniform accelerationStructureEXT";
				debug::join( m_current.text, stmt->getVariable()->getName(), " " );
				m_current.text += ";";
			}

			void visitBufferReferenceDeclStmt( ast::stmt::BufferReferenceDecl * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent;
				m_current.text += "layout(buffer_reference";

				if ( auto structType = getStructType( stmt->getType() ) )
				{
					m_current.text += ", " + debug::getName( structType->getMemoryLayout() );
					m_current.text += ") buffer " + debug::getTypeName( stmt->getType() ) + " { ";
					m_current.text += debug::getTypeName( structType->front().type ) + " " + structType->front().name + debug::getTypeArraySize( structType->front().type ) + "; }";
				}
				else
				{
					m_current.text += ") buffer " + debug::getTypeName( stmt->getType() ) + "s { ";
					m_current.text += debug::getTypeName( stmt->getType() ) + debug::getTypeArraySize( stmt->getType() ) + "; }";
				}
			...
			void visitHitAttributeVariableDeclStmt( ast::stmt::HitAttributeVariableDecl * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent;
				m_current.text += "hitAttributeEXT";
				debug::join( m_current.text, debug::getTypeName( stmt->getVariable()->getType() ), " " );
				debug::join( m_current.text, stmt->getVariable()->getName(), " " );
				m_current.text += ";";
			}
			...
			void visitInOutCallableDataVariableDeclStmt( ast::stmt::InOutCallableDataVariableDecl * stmt )
			{
				...
				m_current.text += m_indent + "layout(" + debug::getLocationName( *var ) + "=" + debug::writeValue( stmt->getLocation() ) + ")";
				debug::join( m_current.text, name, " " );
				debug::join( m_current.text, debug::getTypeName( var->getType() ), " " );
				debug::join( m_current.text, var->getName(), " " );
				m_current.text += ";";
			}
			...
			void visitIfStmt( ast::stmt::If * stmt )
			{
				...
				if ( stmt->getCtrlExpr()->getType()->getKind() != ast::type::Kind::eBoolean )
				{
					m_current.text += m_indent + "if (bool(";
					doSubmit( stmt->getCtrlExpr() );
					m_current.text += "))";
				}
				else
				{
					m_current.text += m_indent + "if (";
					doSubmit( stmt->getCtrlExpr() );
					m_current.text += ")";
				}
				...
				{
					elseIf->accept( this );
					doNextStatement( elseIf.get() );
				}

				if ( stmt->getElse() )
				{
					stmt->getElse()->accept( this );
					doNextStatement( stmt->getElse() );
				}

				m_appendLineEnd = true;
			}

			void visitImageDeclStmt( ast::stmt::ImageDecl * stmt )
			{
				doAppendLineEnd();
				...
				m_current.text += m_indent;
				m_current.text += "layout(";
				m_current.text += debug::getFormatName( image->getConfig().format );
				doWriteBinding( stmt->getBindingPoint()
					, stmt->getDescriptorSet()
					, ", " );
				m_current.text += ") ";
				m_current.text += "uniform ";
				//m_current.text += getAccessQualifierName( image->getConfig() ) + " ";
				m_current.text += debug::getQualifiedName( ast::type::Kind::eImage, image->getConfig() ) + " " + stmt->getVariable()->getName();
				m_current.text += debug::getTypeArraySize( stmt->getVariable()->getType() );
				m_current.text += ";";
			}

			void visitIgnoreIntersectionStmt( ast::stmt::IgnoreIntersection * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent + "ignoreIntersection;";
			}

			void visitInOutVariableDeclStmt( ast::stmt::InOutVariableDecl * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent + debug::getInOutLayout( m_shaderStage, *stmt );
				debug::join( m_current.text, debug::getInterpolationQualifier( *stmt->getVariable() ), " " );
				debug::join( m_current.text, debug::getDirectionName( *stmt->getVariable() ), " " );
				debug::join( m_current.text, debug::getTypeName( stmt->getVariable()->getType() ), " " );
				debug::join( m_current.text, stmt->getVariable()->getName(), " " );
			...
			void visitSpecialisationConstantDeclStmt( ast::stmt::SpecialisationConstantDecl * stmt )
			{
				AST_Failure( "No specialisation constant should remain at this stage" );
			}

			void visitInputComputeLayoutStmt( ast::stmt::InputComputeLayout * stmt )
			{
				doAppendLineEnd();
				...
				if ( stmt->getWorkGroupsZ() == 1 )
				{
					...
					{
						m_current.text += m_indent + "layout(local_size_x=" + debug::writeValue( stmt->getWorkGroupsX() ) + ") in;";
					}
					else
					{
						m_current.text += m_indent + "layout(local_size_x=" + debug::writeValue( stmt->getWorkGroupsX() )
							+ ", local_size_y=" + debug::writeValue( stmt->getWorkGroupsY() ) + ") in;";
					}
				}
				else
				{
					m_current.text += m_indent + "layout(local_size_x=" + debug::writeValue( stmt->getWorkGroupsX() )
						+ ", local_size_y=" + debug::writeValue( stmt->getWorkGroupsY() )
						+ ", local_size_z=" + debug::writeValue( stmt->getWorkGroupsZ() ) + ") in;";
				}
			}

			void visitInputGeometryLayoutStmt( ast::stmt::InputGeometryLayout * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent + "layout(" + debug::getLayoutName( stmt->getLayout() ) + ") in;";
			}

			void visitInputTessellationEvaluationLayoutStmt( ast::stmt::InputTessellationEvaluationLayout * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent + "layout(" + debug::getLayoutName( stmt->getDomain() );
				m_current.text += ", " + debug::getLayoutName( stmt->getPartitioning() );
				m_current.text += ", " + debug::getLayoutName( stmt->getPrimitiveOrdering() );
				m_current.text += ") in;";
			}

			void visitOutputGeometryLayoutStmt( ast::stmt::OutputGeometryLayout * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent + "layout(" + debug::getLayoutName( stmt->getLayout() ) + ", max_vertices = " + debug::writeValue( stmt->getPrimCount() ) + ") out;";
			}

			void visitOutputMeshLayoutStmt( ast::stmt::OutputMeshLayout * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent + "layout(" + debug::getLayoutName( stmt->getTopology() ) + ") out;";
				incLine( m_current );
				m_current.text += m_indent + "layout(max_vertices = " + debug::writeValue( stmt->getMaxVertices() ) + ", max_primitives = " + debug::writeValue( stmt->getMaxPrimitives() ) + ") out;";
			}

			void visitOutputTessellationControlLayoutStmt( ast::stmt::OutputTessellationControlLayout * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent + "layout(vertices=" + debug::writeValue( stmt->getOutputVertices() ) + ") out;";
			}

			void visitPerPrimitiveDeclStmt( ast::stmt::PerPrimitiveDecl * stmt )
			{
			}

			void visitPerVertexDeclStmt( ast::stmt::PerVertexDecl * stmt )
			{
				if ( stmt->getSource() != ast::stmt::PerVertexDecl::Source::eMeshOutput )
				{
					...
					case ast::stmt::PerVertexDecl::Source::eVertexOutput:
						m_current.text += m_indent + "out " + decl + ";";
						break;
			...
			void visitReturnStmt( ast::stmt::Return * stmt )
			{
				doAppendLineEnd();

				if ( stmt->getExpr() )
				{
					m_current.text += m_indent + "return ";
			...
			void visitSampledImageDeclStmt( ast::stmt::SampledImageDecl * stmt )
			{
				doAppendLineEnd();
				auto type = stmt->getVariable()->getType();
				...
				m_current.text += "uniform " + debug::getQualifiedName( ast::type::Kind::eSampledImage, sampledImage->getConfig() ) + " " + stmt->getVariable()->getName();
				m_current.text += debug::getTypeArraySize( stmt->getVariable()->getType() );
				m_current.text += ";";
			}

			void visitCombinedImageDeclStmt( ast::stmt::CombinedImageDecl * stmt )
			{
				doAppendLineEnd();
				auto type = stmt->getVariable()->getType();
				...
				m_current.text += "uniform " + debug::getQualifiedName( ast::type::Kind::eCombinedImage, sampledImage->getConfig(), sampledImage->isComparison() ) + " " + stmt->getVariable()->getName();
				m_current.text += debug::getTypeArraySize( stmt->getVariable()->getType() );
				m_current.text += ";";
			}

			void visitSamplerDeclStmt( ast::stmt::SamplerDecl * stmt )
			{
				doAppendLineEnd();
				m_current.text += m_indent;
				m_current.text += "layout(";
				doWriteBinding( stmt->getBindingPoint()
					, stmt->getDescriptorSet()
					, "" );
				m_current.text += ") uniform sampler";

				if ( static_cast< ast::type::Sampler const & >( *getNonArrayType( stmt->getVariable()->getType() ) ).isComparison() )
				{
					m_current.text += "Shadow";
				}

				m_current.text += " " + stmt->getVariable()->getName();
				m_current.text += debug::getTypeArraySize( stmt->getVariable()->getType() );
				m_current.text += ";";
			}

			void visitShaderBufferDeclStmt( ast::stmt::ShaderBufferDecl * stmt )
			{
				m_appendLineEnd = true;
				doAppendLineEnd();
				m_current.text += m_indent;
				...
				doWriteBinding( stmt->getBindingPoint()
					, stmt->getDescriptorSet()
					, ", " );
				m_current.text += ") buffer " + stmt->getSsboName();
				m_appendSemiColon = false;
				visitCompoundStmt( stmt );
				m_current.text += ";";
				m_appendLineEnd = true;
			...
			void visitShaderStructBufferDeclStmt( ast::stmt::ShaderStructBufferDecl * stmt )
			{
				m_appendLineEnd = true;
				doAppendLineEnd();
				m_current.text += m_indent;
				...
				m_current.text += "{";
				incLine( m_current );
				m_current.text += "\t" + debug::getTypeName( arrayType->getType() ) + " " + data->getName();
				m_current.text += debug::getTypeArraySize( arrayType ) + ";";
				incLine( m_current );
				m_current.text += "} " + stmt->getSsboInstance()->getName() + ";";
				m_appendLineEnd = true;
			}
			...
			void visitStructureDeclStmt( ast::stmt::StructureDecl * stmt )
			{
				m_appendLineEnd = true;
				doAppendLineEnd();
				debug::writeStruct( m_indent, *stmt->getType(), std::string{}, m_current );
			}

			void visitSwitchCaseStmt( ast::stmt::SwitchCase * stmt )
			{
				doAppendLineEnd();

				if ( stmt->getCaseExpr() )
				{
					m_current.text += m_indent + "case ";
					doSubmit( stmt->getCaseExpr() );
					m_current.text += ":";
				}
				else
				{
					m_current.text += m_indent + "default:";
				}
			...
			void visitSwitchStmt( ast::stmt::Switch * stmt )
			{
				m_appendLineEnd = true;
				doAppendLineEnd();
				m_current.text += m_indent + "switch (";
			...
			void visitTerminateRayStmt( ast::stmt::TerminateRay * stmt )
			{
				m_current.text += m_indent + "terminateRay;";
			}

			void visitVariableDeclStmt( ast::stmt::VariableDecl * stmt )
			{
				if ( !stmt->getVariable()->isBuiltin() )
				{
					doAppendLineEnd();
					...
					if ( var->isPerTaskNV() )
					{
						...
						{
							m_current.text += m_indent + "taskNV";
							debug::join( m_current.text, debug::getDirectionName( *var ), " " );
							m_current.text += " ";
							debug::writeBlock( m_indent, *structType, m_current );
							debug::join( m_current.text, var->getName(), " " );
							m_current.text += ";";
						}
					}
					else if ( var->isPerTask() )
					{
						...
						if ( structType && !structType->empty() )
						{
							m_appendLineEnd = true;
							doAppendLineEnd();
							debug::writeStruct( m_indent, *structType, std::string{}, m_current );
						}

						if ( !structType || !structType->empty() )
						{
							m_current.text += m_indent + "taskPayloadSharedEXT";
							debug::join( m_current.text, debug::getTypeName( var->getType() ), " " );
							debug::join( m_current.text, var->getName(), " " );
							m_current.text += ";";
						}
					...
					{
						debug::join( m_current.text, debug::getDirectionName( *var ), " " );
						debug::join( m_current.text, debug::getInterpolationQualifier( *var ), " " );
						debug::join( m_current.text, debug::getTypeName( var->getType() ), " " );
						debug::join( m_current.text, var->getName(), " " );
						m_current.text += debug::getTypeArraySize( var->getType() );
						m_current.text += ";";
						m_current.text = m_indent + m_current.text;
					}
				}
			}

			void visitWhileStmt( ast::stmt::While * stmt )
			{
				m_appendLineEnd = true;
				doAppendLineEnd();
			...
			void visitPreprocDefine( ast::stmt::PreprocDefine * preproc )
			{
				doAppendLineEnd();
				m_current.text += "#define " + preproc->getName() + " ";
				doSubmit( preproc->getExpr() );
			}

			void visitPreprocElif( ast::stmt::PreprocElif * preproc )
			{
				doAppendLineEnd();
				m_current.text += "#elif ";
			...
			void visitPreprocElse( ast::stmt::PreprocElse * preproc )
			{
				doAppendLineEnd();
				m_current.text += "#else";
			}

			void visitPreprocEndif( ast::stmt::PreprocEndif * preproc )
			{
				doAppendLineEnd();
				m_current.text += "#endif";
			}

			void visitPreprocExtension( ast::stmt::PreprocExtension * preproc )
			{
			}

			void visitPreprocIf( ast::stmt::PreprocIf * preproc )
			{
				doAppendLineEnd();
				m_current.text += "#if ";
				doSubmit( preproc->getCtrlExpr() );
			...
			void visitPreprocIfDef( ast::stmt::PreprocIfDef * preproc )
			{
				doAppendLineEnd();
				m_current.text += "#ifdef ";
				doSubmit( preproc->getIdentExpr() );
			}

			void visitPreprocVersion( ast::stmt::PreprocVersion * preproc )
			{
			}
			...
			void doWriteBinding( uint32_t binding
				...
				, std::string sep )
			{
				if ( binding != debug::InvalidIndex )
				{
					m_current.text += sep + "binding=" + debug::writeValue( binding );
					sep = ", " + sep + "set=" + debug::writeValue( set );
				}
			}
